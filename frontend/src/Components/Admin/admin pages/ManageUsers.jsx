...
const ManageUsers = () => {
  ...
  const handlePromote = async (userId) => {
    ...
    try {
      ...
      // Admins can only promote to vendor
      if (newRole !== "vendor") {
        setRoleError("Admins can only promote to vendor");
        setActionLoading(false);
        return;
      }
      await axios.put(
  ...
  // Button to navigate to Manage Vendor Requests
  const handleVendorRequests = () => {
    navigate("/admin/managevendorrequests");
  };
  ...
  return (
    <div>
      ...
      <div className="ml-64 flex flex-col min-h-screen bg-gray-50">
        <AdminTopbar />
        <div className="p-8 pt-24"> {/* pt-24 ensures content is pushed below topbar */}
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <h1 className="text-2xl font-bold">Manage Users</h1>
            <button
              onClick={handleVendorRequests}
              className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded shadow transition whitespace-nowrap"
              style={{ zIndex: 10 }}
            >
